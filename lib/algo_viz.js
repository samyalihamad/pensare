...
(function (global) {
  ...
  function buildTree(data) {
    var start = data.start, steps = data.steps;
    var parent = {}; parent[start] = null;
    steps.forEach(function (s) {
      (s.discovered || []).forEach(function (u) {
        if (!(u in parent)) parent[u] = s.current;
      });
    });
    var nodes = {}; // url -> node
    ...
    function depth(n) { var d = 0, c = n; while (c.parent != null && nodes[c.parent]) { d++; c = nodes[c.parent]; } return d; }
  ...
  function renderAlgoViz(data, mount) {
    ...
    var W = tree.width * COLW, H = (tree.maxDepth + 1) * ROWH + PAD * 2 - (ROWH - 50);
    function px(node) { return PAD + node.x * COLW + COLW / 2 - (tree.width ? 0 : 0); }
    ...
    mount.innerHTML =
      ...
      '<div class="av-body">' + codeHtml +
        '<div class="av-main">' +
          '<div class="av-tree"><svg viewBox="0 0 ' + W + ' ' + H + '" preserveAspectRatio="xMidYMid meet">' +
